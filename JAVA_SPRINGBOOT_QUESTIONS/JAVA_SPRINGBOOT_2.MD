...
public class BuilderVsBufferDemo {
    public static void main(String[] args) throws InterruptedException {
        ...
        // Expected 2005 (Start + 1000A + 1000B) but often smaller/garbled
...
1. StringBuilder case:  
  a. Both threads (t1, t2) are appending to the same builder.  
  b. Since it’s not synchronized, sometimes they overwrite each other’s writes.  
  c. Result: corrupted buffer or wrong length (e.g., 1980 instead of 2005).  

2. StringBuffer case:  
  a. Same scenario, but all methods (append, insert, etc.) are synchronized.  
  b. Only one thread modifies at a time.  
  c. Result: always correct length = 2005.  

3. Conclusion:  
  a. Use StringBuilder in single-threaded situations (faster).  
  b. Use StringBuffer in multi-threaded situations (safe but slower).  
...
1. What is it?  
  a. The String pool is a special area inside the Java heap (in Method Area / Metaspace depending on JVM).  
  b. JVM maintains a cache of String literals there.  
  c. If two String literals have the same value, they are made to point to the same object in the pool (to save memory).  

2. Example: String Literals (Interning)  
```java
public class Test {
    public static void main(String[] args) {
        String s1 = "Hello";  // goes into String pool
        String s2 = "Hello";  // refers to the same pool object
...
```
Both s1 and s2 point to the same pool object → so == is true.

3. Example: new String()  
```java
public class Test {
    public static void main(String[] args) {
        String s1 = "Hello";               
        String s2 = new String("Hello");   // forces a new object in heap
...
```
```java
Even though both have "Hello" as content,
s1 → points to String pool.
s2 → points to a new heap object.
So == is false but .equals() is true.
```

4. Forcing Pooling with .intern()  
```java
public class Test {
    public static void main(String[] args) {
...
```
.intern() → ensures the string reference is from the pool.

5. Summary  
  a. String literals are automatically interned → stored in the String pool.  
  b. When you write "abc" == "abc", both refer to the same pool object → == returns true.  
  c. When you write new String("abc"), a new object is created in heap → == returns false even though .equals() is true.  
  d. Use .intern() if you want heap strings to be moved into the pool.  
...
```java
          +-------------------+
          |   String Pool     |   (inside Heap/Metaspace)
          +-------------------+
          |   "Hello"   <---- s1, s2, s4
          +-------------------+
...
2. Step by Step: -  
  a. String s1 = "Hello"; 
  b. "Hello" is created in the String pool.  
  c. s1 points to it.  
  d. String s2 = "Hello";  
  e. "Hello" already exists in the pool.  
  f. s2 points to the same pooled object as s1.  
  g. s1 == s2 → true.  
  h. String s3 = new String("Hello");  
  i. A new String object is created in the heap (not pool).  
  j. s3 points to this new object.  
  k. s1 == s3 → false (different references).  
  l. String s4 = s3.intern();  
  m. .intern() checks the pool for "Hello".  
  n. Since it exists, s4 points to the pooled "Hello".  
  o. s4 == s1 → true.  

3. Quick Summary  
a. String Pool → special memory area for string literals (saves memory).  
b. Heap → new objects created via new String().  
c. "abc" == "abc" → true (same pool reference).  
d. "abc".equals("abc") → true (same content).  
e. new String("abc") == "abc" → false (different references).  
f. Use .intern() to force pooling.  
...
| **String Pool**   |  Uses pool             |  No pool             |  No pool                     |
| **Thread Safety** | N/A (immutable = safe) |  Not thread-safe     |  Thread-safe (synchronized)  |
| **equals()**      |  Compares content      |  Compares reference  |  Compares reference          |
| **Performance**   | Medium                 | Fast (no sync)       | Slower (sync overhead)       |
