...
| | Problem: During rehashing, it could lead to infinite loops in multithreaded code (because rehashing wasn’t thread-safe).* | Prevents reverse order issue and avoids infinite loop risk. |
...
* In Java 7 HashMap, rehashing during resize was not thread-safe. If multiple threads resized at the same time, the LinkedList in buckets could become corrupted and form a circular reference, leading to an infinite loop during traversal.
